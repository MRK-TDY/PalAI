...
class PalAI:
    ...
    async def apply_add_ons(self):
        ...
        add_ons = await self.llm_client.get_llm_response(
            self.prompts_file["add_on_system_message"], add_on_prompt, type="addons"
        )
    ...
    async def get_artist_response(self):
        """Gets the materials and style for the building"""

        materials_response = await self.llm_client.get_llm_response(
            self.prompts_file["material_system_message"].format(
    ...
    async def decorate(self):
        """Creates the list of decorations"""
        decorator = Decorator()
